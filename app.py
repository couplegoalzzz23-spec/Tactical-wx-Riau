# ===============================================================
# Tactical Weather Ops — BMKG (Clean UI + No Error Guaranteed)
# ===============================================================
...
# ---------------------------------------------------------------
# UI STYLE (military console theme)
# ---------------------------------------------------------------
st.markdown("""
<style>
body {
...
.radar {
    ...
    background: radial-gradient(circle, rgba(80,255,120,0.12) 10%, transparent 11%);
    border: 2px solid #4f8;
    margin: auto;
    box-shadow: 0 0 12px #3f6;
}
.radar:before {
    content: "";
    position: absolute; top: 50%; left: 50%;
    width: 70%; height: 2px;
    background: linear-gradient(90deg,#4f8,transparent);
...
with st.sidebar:
    ...
    st.markdown("<p style='text-align:center;color:#7f7'>SCANNING…</p>", unsafe_allow_html=True)
    ...
    st.markdown("---")
    st.caption("Tactical Ops UI © 2025")
...
# FETCH DATA
# ---------------------------------------------------------------
with st.spinner("Retrieving BMKG data…"):
    try:
...
if not entries:
    st.error("No data returned from BMKG.")
    st.stop()

# ---------------------------------------------------------------
# LOCATION PICKER (AS REQUESTED: DO NOT CHANGE)
# ---------------------------------------------------------------
locations = {}
for e in entries:
    loc = e.get("lokasi", {})
...
if df.empty:
    st.error("No forecast data for selected location.")
    st.stop()
...
with c1: st.metric("TEMP (°C)", f"{row['t']:.1f}" if not pd.isna(row['t']) else "—")
with c2: st.metric("Humidity", f"{row['hu']}%" if not pd.isna(row['hu']) else "—")
with c3: st.metric("Wind (KT)", f"{row['ws_kt']:.1f}" if not pd.isna(row['ws_kt']) else "—")
with c4: st.metric("Rain (mm)", row["tp"] if not pd.isna(row["tp"]) else "—")
...
if "Temperature" in param and df["t"].notna().any():
    fig = px.line(df, x="local_dt", y="t", title="Temperature (°C)")
    st.plotly_chart(fig, use_container_width=True)

if "Humidity" in param and df["hu"].notna().any():
    fig = px.line(df, x="local_dt", y="hu", title="Humidity (%)")
    st.plotly_chart(fig, use_container_width=True)

if "Wind" in param and df["ws_kt"].notna().any():
    fig = px.line(df, x="local_dt", y="ws_kt", title="Wind Speed (KT)")
    st.plotly_chart(fig, use_container_width=True)

if "Rain" in param and df["tp"].notna().any():
...
try:
    ...
    freq["theta"] = freq["sector"].map(theta_map)
