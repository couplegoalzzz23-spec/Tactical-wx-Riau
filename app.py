...
""", unsafe_allow_html=True)
# =====================================
# 🛰️ SATELLITE IMAGERY (OPTIONAL ADD-ON)
...
# Toggle di sidebar (aman walau sidebar sudah ada)
with st.sidebar:
    show_satellite = st.checkbox("Show Satellite Imagery", value=False)
...
if show_satellite:
    ...
    # -----------------------------
    # 🌍 HIMAWARI-8 IR (INFRARED)
    # -----------------------------
    with col_sat1:
        st.markdown("### 🌑 Himawari-8 Infrared (IR)")
        try:
            himawari_ir_url = (
                "https://inderaja.bmkg.go.id/IMAGE/HIMA/H08_IR1.jpg"
            )
            st.image(
                himawari_ir_url,
                caption="BMKG Himawari-8 Infrared — Cloud Top Temperature",
                use_container_width=True
            )
            st.caption("🛈 Reference only — not for tactical separation.")
        except Exception as e:
            st.warning(f"Infrared satellite unavailable: {e}")

    # -----------------------------
    # ☁️ HIMAWARI-8 VIS / CLOUD
    # -----------------------------
    with col_sat2:
        st.markdown("### ☁️ Himawari-8 Cloud / VIS")
        try:
            himawari_vis_url = (
                "https://inderaja.bmkg.go.id/IMAGE/HIMA/H08_VIS.jpg"
            )
            st.image(
                himawari_vis_url,
                caption="BMKG Himawari-8 Visible / Cloud Cover",
                use_container_width=True
            )
            st.caption("🛈 Daytime only — visual cloud structure.")
        except Exception as e:
            st.warning(f"Visible satellite unavailable: {e}")

    # -----------------------------
    # ⚠️ DISCLAIMER OPERASIONAL
    # -----------------------------
    st.markdown("""
    <div style="background:#081108; padding:12px; border-radius:8px; border:1px solid #224422; font-size:0.85rem;">
    <strong>Operational Notice:</strong><br>
    Satellite imagery is provided for <em>situational awareness only</em>.<br>
